...
/// <summary>
/// Locates the repository's <c>src</c> tree and scans it for <c>&lt;example&gt;&lt;code&gt;</c>
/// blocks in <c>///</c> documentation comments.
/// </summary>
public static class DocExampleSource
{
    /// <summary>
    /// Finds every <c>&lt;example&gt;&lt;code&gt;</c> block under <c>src/</c>.
    /// </summary>
    ...
    //
    // Scans the whole `src` tree (not just EntityFrameworkCore) because examples live in
    ...
    private static string LocateSourceDirectory()
    {
        ...
        while (directory is not null)
        {
            ...
            var anchor = Path.Combine(candidate, "Wolfgang.AuditTrail.Abstractions", "Wolfgang.AuditTrail.Abstractions.csproj");
            if (Directory.Exists(candidate) && System.IO.File.Exists(anchor))
            {
                return candidate;
            }
